...
const Spotify = {
    ...
    authorizeSpotify() {
        ...
        if (accessTokenMatch) {
            ...
            return accessToken;
        } else { // Direct user to the login page of spotify.
    ...
    searchSong(query) {
        ...
        }).then(response => { // Response comes back in a json
    ...
    getRecommendations() {
        ...
        const {term} = this.state;
            return fetch(`https://api.spotify.com/v1/search?q=${term}&type=playlist&limit=20`, {
            headers: { 
                Authorization: `Bearer ${accessToken}`
            }
          }).then(response => {
